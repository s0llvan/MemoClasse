import { Component } from '@angular/core';
import { IonicPage, NavController,Platform,NavParams } from 'ionic-angular';
import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
pdfMake.vfs = pdfFonts.pdfMake.vfs;

import { File } from '@ionic-native/file';
import { FileOpener } from '@ionic-native/file-opener';
import { ToastController } from 'ionic-angular';
import { EmailComposer } from '@ionic-native/email-composer';
import { Base64 } from '@ionic-native/base64';

@IonicPage()
@Component({
  selector: 'page-pdf',
  templateUrl: 'pdf.html',
})
export class PdfPage {
  letterObj = {
    activite: '',
    text: '',
    addSub: '',
    addText: ''
  }

  pdfObj = null;
 student = null;
  //Le string, venant des photos par exemple

    base64string = ""

  docDefinition = {
    content: [
      { text: "Rapport de l'élève Xxxx Xxxx", style: 'header' },
      { text: new Date().toTimeString(), alignment: 'right' },
      { text: 'Classe Xx', style: 'subheader' },
      { text: 'Enseignant : ', style: 'subheader' },
      { text: 'Activité : '+  this.letterObj.activite, style: 'subheader' },


      {
      image: "data:image/png;base64,"+this.base64string,style:'image'  //marche si base64string est correct
      },

      {
      image: "data:image/png;base64,"+this.base64string,style:'image'
      },


    ],
    styles: {
      header: {
        fontSize: 18,
        bold: true,
      },
      subheader: {
        fontSize: 14,
        bold: true,
      },
      image: {
        width:200,
        margin: [100, 15, 0, 0]
      },
      text: {
        fontSize: 11
      }
    }

  }

  constructor(public navParams: NavParams,private base64: Base64,public navCtrl: NavController, public plt: Platform, private file: File, private fileOpener: FileOpener,private toastCtrl: ToastController,private emailComposer: EmailComposer) { this.student = this.navParams.get('student') }

  createPdf() {

    this.pdfObj = pdfMake.createPdf(this.docDefinition);
    this.presentToast();
  }

addImage(){
  this.base64string = this.student.pictures[0];
      this.docDefinition.content.push({text: this.base64string,style:'text'},);
}

addImageLink(base64image){
      this.docDefinition.content.push({image: 'data:image/png;base64,'+base64image,style:'image'},);
}

addChamps()
{
  this.addActivite();
  this.addSubHeader();
  this.addText();
  this.addImage();
}

addActivite(){
  let ac = this.letterObj.activite;
      this.docDefinition.content.push({text: ac,style:'header'},);
}

addSubHeader(){
  let head = this.letterObj.addSub;
      this.docDefinition.content.push({text: head,style:'subheader'},);
}

addText(){
  let text = this.letterObj.addText;
      this.docDefinition.content.push({text: text,style:'text'},);
}

  openPdf() {
    if (this.plt.is('cordova')) {
      this.pdfObj.getBuffer((buffer) => {
        var blob = new Blob([buffer], { type: 'application/pdf' });
        // Save the PDF to the data Directory of our App
        this.file.writeFile(this.file.dataDirectory, 'rapportEleve.pdf', blob, { replace: true }).then(fileEntry => {
          // Open the PDf with the correct OS tools
          this.fileOpener.open(this.file.dataDirectory + 'rapportEleve.pdf', 'application/pdf');
        })
      });
    } else {
      // On a browser simply use download!
      this.pdfObj.download();
    }
  }

  presentToast() {
  let toast = this.toastCtrl.create({
    message: 'Le pdf a bien été généré',
    duration: 3000,
    position: 'top',
    cssClass: "toast"
  });
  toast.present();
}



sendMail() {
  let email = {
    to: "test@mail.fr",

    attachments: [
     'file://assets/imgs/nx.png'
    ],
    subject: 'Cordova Icons',
    body: 'How are you? Nice greetings from Leipzig',
    isHtml: true
  };

  // Send a text message using default options
  this.emailComposer.open(email);
}



}
